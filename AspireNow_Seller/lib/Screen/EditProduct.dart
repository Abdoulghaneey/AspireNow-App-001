...
late List<String> showOtherImages = [];
...
//========================= Simple Product =====================================
    ...
    if (productType == "simple_product") {
      ...
      if (widget.model!.attributeList!.isEmpty.toString() == "false") {
        ...
        for (int i = 0; i < index; i++) {
          ...
          oldListOfAttributeValueID.forEach(
            (element) {
              final tempvar =
          ...
          for (int i = 0; i < index; i++) {
            ...
            oldVariantId = () {
              if (oldVariantId == "") {
                ...
                return oldVariantId! +
                    "," +
                    widget.model!.prVarientList![i].id!;
              }
            }();
          }
        }
    ...
    if (productType == "variable_product") {
      ...
      if (widget.model!.stockType == "1") {
        print("variation list StockType == product level");
        ...
        print("variantion Length :");
        print(variationList.length);
        variationList.clear();
        print("variantion Length :");
        print(variationList.length);
        print(
            "widget.model!.prVarientList!.length : ${widget.model!.prVarientList!.length}");
        for (int i = 0; i < widget.model!.prVarientList!.length; i++) {
          variationList.add(widget.model!.prVarientList![i]);
          colCount = variationList[i].attr_name!.split(',');
        }
        variationList.forEach(
          (element) {
            print("1");
            print(element.attr_name);
            print("2");
            print(element.attribute_value_ids);
            print("3");
            print(element.availability);
            print("4");
            print(element.cartCount);
            print("5");
            print(element.disPrice);
            print("6");
            print(element.id);
            print("7");
            print(element.price);
      ...
      if (widget.model!.stockType == "2") {
        print("variation list StockType == variant level ");
        ...
        if (widget.model!.attributeList!.isEmpty.toString() == "false") {
          ...
          for (int i = 0; i < index; i++) {
            ...
            //old variant id
            String? oldattributename = widget.model!.attributeList![i].name;
            print("oldAttributename : ${widget.model!.attributeList![i].name}");
            _attrController.add(
              TextEditingController(text: oldattributename),
            );
            variationBoolList.add(true);
...
//variation section
            curSelPos == 2 && variationList.isNotEmpty
                ? ListView.builder(
                    ...
                    itemBuilder: (context, i) {
                      print(
                          "variantList[$i].name : ${variationList[i].attr_name!}");
                      // print(
                      //     "variantList[$i].id : ${variationList[i].attribute_value_ids!}");
  ...
  getCombination(List<String> att, List<String> attId, int i) {
    for (int j = 0, l = finalAttList[i].length; j < l; j++) {
      ...
      print("final Attribute value : ");
      print(finalAttList[i][j].value!);
      if (i == max) {
        resultAttr.addAll(a);
        resultID.addAll(aId);
  ...
  _buildExpandableContent(int pos) {
    ...
    variationList.forEach(
      (element) {
        print("===========>>>>>>==============");
        print("name : ${element.attr_name}");
        print("value : ${element.varient_value}");
        print("value of attrobute : ");
        print(element.attribute_value_ids);
        print(element.availability);
        print(element.cartCount);
    ...
    if (_isNetworkAvail) {
      try {
        ...
        if (productType == 'simple_product') {
          ...
          print(" variationList.length :  ${variationList.length}");
          for (int i = 0; i < variationList.length; i++) {
            // print("Attribute Value Id [$i] :");
            // print(variationList[i].attribute_value_ids.toString());
          ...
          print("variant Id : ${request.fields[VariantsIds].toString()}");
          print(oldVariantId);
          print(val);
...
//=========================== Body Part ========================================
  ...
  void validateAndSubmit() async {
    ...
    attributeIds.forEach(
      (key) {
        selectedAttributeValues[key]!.forEach(
          (element) {
