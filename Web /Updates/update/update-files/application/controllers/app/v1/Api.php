...
*/
    ...
    public function __construct()
    {
        ...
        $this->load->model(['category_model', 'order_model', 'rating_model', 'Area_model', 'cart_model', 'address_model', 'transaction_model', 'ticket_model', 'Order_model', 'notification_model', 'faq_model', 'Seller_model', 'Promo_code_model', 'media_model']);
  ...
  */
    ...
    */
    ...
    {
        $settings = get_settings('system_settings', true);
        $upload_media = isset($settings['upload_media']) ? $settings['upload_media'] : '';
        ...
        if ($upload_media == 1) {
            ...
            if ($limit <= $upload_limit) {
                ...
                if (empty($_FILES) || $other_image_info_error != NULL) {
                    $this->response['error'] = true;
                    $this->response['message'] = (empty($_FILES)) ? "Files does not Uploaded Successfully..!" :  $other_image_info_error;
                    print_r(json_encode($this->response));
                } else {
                    $this->response['error'] = false;
